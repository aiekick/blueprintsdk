...
struct ExposureNode final : Node
{
    ...
    FlowPin Execute(Context& context, FlowPin& entryPoint, bool threading = false) override
    {
        ...
        if (!mat_in.empty())
        {
            ...
            if (mat_in.device == IM_DD_VULKAN)
            {
                ...
                m_filter->filter(in_RGB, im_RGB, m_exposure);
            ...
            {
                m_filter->filter(mat_in, im_RGB, m_exposure);
                im_RGB.time_stamp = mat_in.time_stamp;
                im_RGB.rate = mat_in.rate;
                im_RGB.flags = mat_in.flags;
                m_MatOut.SetValue(im_RGB);
    ...
    bool DrawCustomLayout(ImGuiContext * ctx, float zoom, ImVec2 origin) override
    {
        ...
        if (ImGui::Checkbox("##enable_filter_Exposure",&check)) { m_bEnabled = check; changed = true; }
        ImGui::SameLine(); ImGui::TextUnformatted("Exposure");

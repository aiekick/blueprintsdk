...
struct SaturationNode final : Node
{
    ...
    SaturationNode(BP& blueprint): Node(blueprint) { m_Name = "Mat Saturation"; }
    ...
    FlowPin Execute(Context& context, FlowPin& entryPoint, bool threading = false) override
    {
        ...
        if (!mat_in.empty())
        {
            ...
            m_filter->filter(mat_in, im_RGB, m_saturation);
    ...
    bool DrawCustomLayout(ImGuiContext * ctx, float zoom, ImVec2 origin) override
    {
        ...
        if (ImGui::ToggleButton("##enable_filter_Saturation",&check)) { m_bEnabled = check; changed = true; }
